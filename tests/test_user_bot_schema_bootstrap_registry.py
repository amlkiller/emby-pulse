...
def test_user_bot_bootstrap_creates_registry_and_local_tables(monkeypatch, tmp_path):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        assert {"tg_bot_users", "tg_channel_bindings"}.issubset(existing_tables)
        assert {
            "tg_user_id",
            "tg_username",
            "tg_display_name",
...
def test_user_bot_bootstrap_uses_schema_registry_for_owned_tables_only():
    ...
    assert "TABLE_SCHEMAS[table_name]" in source
    assert "TABLE_ALTERS.get(table_name, [])" in source

    for registry_owned_table in ("tg_user_bindings", "tg_user_blacklist", "tg_reg_logs"):
    ...
    assert "CREATE TABLE IF NOT EXISTS tg_bot_users" in source
    assert "CREATE TABLE IF NOT EXISTS tg_channel_bindings" in source
