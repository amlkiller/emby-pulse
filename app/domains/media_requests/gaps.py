...
from app.domains.media_requests import gap_download_service
...
gap_download_service.set_dependency_providers(
    ...
    gap_config_map_provider=lambda: get_gap_config_map(),
    save_gap_record_status_provider=lambda: save_gap_record_status,
    scan_state_provider=lambda: scan_state,
    state_lock_provider=lambda: state_lock,
    user_is_admin_provider=lambda request: user_service.is_admin_user(request),
    thread_factory_provider=lambda: threading.Thread,
    hook_qbittorrent_provider=lambda: hook_qbittorrent,
    hook_transmission_provider=lambda: hook_transmission,
)
...
def extract_episodes_from_filename(filename: str) -> set:
    return gap_download_service.extract_episodes_from_filename(filename)


def hook_qbittorrent(host, user, password, expected_size, target_episodes, torrent_name=None):
    return gap_download_service.hook_qbittorrent(host, user, password, expected_size, target_episodes, torrent_name)


def hook_transmission(host, user, password, expected_size, target_episodes):
    return gap_download_service.hook_transmission(host, user, password, expected_size, target_episodes)


@router.post("/download")
def download_gap_item(request: Request = None, payload: dict = None):
    return gap_download_service.download_gap_item(request, payload)
