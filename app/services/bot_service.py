...
class NotificationBot:
    ...
    def _cmd_stats(self, chat_id, period='day', platform="tg"):
        ...
        try:
            plays_res = query_db(f"SELECT COUNT(*) as c FROM PlaybackActivity {where}{exclude_sql}", params)
            if not plays_res: raise Exception("DB Error")
            plays = plays_res[0]['c']
            dur_res = query_db(f"SELECT SUM(PlayDuration) as c FROM PlaybackActivity {where}{exclude_sql}", params)
            dur = dur_res[0]['c'] if dur_res and dur_res[0]['c'] else 0
            # 🔥 使用格式化字符串，确保四舍五入一致
            hours_str = f"{dur / 3600:.1f}"
            users_res = query_db(f"SELECT COUNT(DISTINCT UserId) as c FROM PlaybackActivity {where}{exclude_sql}", params)
            ...
            # 用户排行
            top_users = query_db(f"SELECT UserId, SUM(PlayDuration) as t FROM PlaybackActivity {where}{exclude_sql} GROUP BY UserId ORDER BY t DESC LIMIT 5", params)
            user_str = ""
            if top_users:
            ...
            # 🔥 内容排行 - 区分剧集和电影，按时长排序
            all_content = query_db(f"SELECT ItemName, ItemId, ItemType, COUNT(*) as C, COALESCE(SUM(PlayDuration), 0) as Duration FROM PlaybackActivity {where}{exclude_sql} GROUP BY ItemName ORDER BY Duration DESC LIMIT 100", params)
