...
- Good: `app.infra.db.database.init_db()` routes low-risk registry-owned compatibility tables without special migration semantics through `_REGISTRY_COMPAT_SIMPLE_INIT_TABLES` plus `schema_bootstrap.ensure_registered_table(...)`, while leaving high-risk media request compatibility DDL local until its own slice.
