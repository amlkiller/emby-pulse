...
- Good: `bot_service.py` keeps Emby availability/Policy checks, Telegram callback handling, message editing, notification dispatch, gap scan-state mutation, and text rendering in the service while delegating request-admin message sync persistence, notify-rule/mute reads/bootstrap, media request/feedback status persistence, gap table/cache persistence, message reply/block persistence, playback read queries, and user expiration metadata reads to DAO/query boundaries.
