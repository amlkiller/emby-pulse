...
- Good: `app.infra.db.session_dao.ensure_session_table()` creates registry-owned `sessions` through `schema_bootstrap.ensure_registered_table(...)` while keeping the session expiry index local until index metadata is centralized.
...
- Focused session bootstrap test: run `ensure_session_table()` against a temporary database and assert registry-backed `sessions` creation, preserved expiry index, create/get/update/delete/clear/cleanup DAO smoke paths, and no local duplicate session table DDL in the DAO source.
