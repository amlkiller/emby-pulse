...
- New local `CREATE TABLE IF NOT EXISTS sys_notifications`, `CREATE TABLE IF NOT EXISTS sys_dashboard`, `CREATE TABLE IF NOT EXISTS sessions`, or `CREATE TABLE IF NOT EXISTS audit_logs` in small bootstrap helpers -> fail focused bootstrap/schema registry tests.
...
- New local `media_requests`, `request_users`, or `media_feedback` table DDL or compatible local `media_requests` / `media_feedback` ALTER statements in `app.domains.media_requests.media_request_dao.ensure_media_request_schema()` -> fail focused media-request bootstrap/schema registry tests.
...
- Good: `app.domains.system.system_tool_dao.repair_core_system_tables()` creates repaired registry-owned tables from `TABLE_SCHEMAS` / `PLAYBACK_SCHEMA`, applies `TABLE_ALTERS`, and applies registered `TABLE_INDEXES`.
- Good: `app.infra.db.database._create_system_tables()` routes simple registry-owned startup tables, including `media_requests`, through `_REGISTRY_SYSTEM_INIT_TABLES` plus `schema_bootstrap.ensure_registered_table(...)`, while keeping unregistered or high-risk migration logic local until its own slice.
...
- Good: `app.infra.db.database._create_system_tables()` creates the registry-owned `tv_series_status` table through `_REGISTRY_SYSTEM_INIT_TABLES` plus `schema_bootstrap.ensure_registered_table(...)`, while calendar status reads and writes stay in `app.domains.playback.calendar_dao`.
- Good: `app.infra.db.database._create_system_tables()` creates `login_failures` and `api_tokens` through `_REGISTRY_SYSTEM_INIT_TABLES` plus `schema_bootstrap.ensure_registered_table(...)`; login and token lookup indexes come from `TABLE_INDEXES`.
- Good: `app.infra.db.notification_dao.ensure_notifications_table()` uses `schema_bootstrap.ensure_registered_table(...)` so CREATE, registered ALTER, and registered indexes stay on the shared bootstrap path.
- Good: `app.domains.system.system_tool_dao` dashboard helpers use `schema_bootstrap.ensure_registered_table(...)` for `sys_dashboard`, while `repair_core_system_tables()` keeps its explicit registry-metadata repair path.
- Good: `app.infra.db.session_dao.ensure_session_table()` creates registry-owned `sessions` through `schema_bootstrap.ensure_registered_table(...)`; the session expiry index is declared in `TABLE_INDEXES`.
- Good: `app.infra.db.audit_logger_dao.ensure_audit_table()` creates registry-owned `audit_logs` through `schema_bootstrap.ensure_registered_table(...)`; audit-log indexes are declared in `TABLE_INDEXES`.
- Good: `app.domains.media_requests.gap_dao.ensure_gap_tables()` loops its registry-owned `gap_*` table subset through `schema_bootstrap.ensure_registered_table(...)` and keeps only default config insertion plus the legacy `gap_scan_cache` rebuild decision local.
- Good: `app.domains.playback.dedupe_dao.init_dedupe_tables()` creates and upgrades registry-owned `dedupe_*` tables through `schema_bootstrap.ensure_registered_table(...)` and keeps only the legacy whitelist data-copy migration logic local.
- Good: small notification/message bootstrap helpers create registry-owned request-admin, notify-rule, bot-mute, message, notify-block, user-mute, and announcement tables through `schema_bootstrap.ensure_registered_table(...)`; simple indexes such as `idx_request_admin_messages_tmdb` are declared in `TABLE_INDEXES`.
...
- Good: `app.domains.users.auth_dao.ensure_local_users_table()` creates and upgrades registry-owned `local_users` through `schema_bootstrap.ensure_registered_table(...)`, applying only safe optional-column entries registered in `TABLE_ALTERS["local_users"]` and leaving unsafe legacy repairs such as adding `UNIQUE NOT NULL` identity columns out of generic ALTER metadata.
- Good: `app.domains.system.pro_license_dao.ensure_pro_schema()` creates and upgrades registry-owned `sys_license` through `schema_bootstrap.ensure_registered_table(...)`, keeping nullable extension columns such as `max_devices` and `current_devices` compatible with existing inserts that write only license key, machine id, and status.
...
- Good: `app.infra.db.database.init_db()` creates and upgrades `PlaybackActivity` through `schema_bootstrap.ensure_playback_table(...)`; playback indexes are declared in `TABLE_INDEXES["PlaybackActivity"]`.
...
- Good: `app.plugins.plugin_dao.ensure_plugin_tables()` creates registry-owned `plugin_state` and `plugin_logs` tables through `schema_bootstrap.ensure_registered_table(...)`; the plugin-log index is declared in `TABLE_INDEXES`.
...
- Focused ownership test: assert `app.infra.db.schema_registry` contains the schema metadata definitions, including `TABLE_INDEXES`, and does not import `app.core.db_schemas`.
- Focused identity test: assert `app.infra.db.database.SYSTEM_TABLES is app.infra.db.schema_registry.SYSTEM_TABLES`.
- Focused repair test: run `repair_core_system_tables()` against a temporary database and assert registry-backed table creation plus registered ALTER and registered index application.
- Focused database-init test: run `init_system_db()` against a temporary database and assert `_REGISTRY_SYSTEM_INIT_TABLES` tables are created from registry metadata, registered ALTER columns exist, registered indexes exist, and local DDL remains only for unregistered/high-risk tables.
...
- Focused auth/API-token database-init test: run `init_system_db()` against a temporary database and assert registry-backed `login_failures` / `api_tokens` creation, registered login/token indexes from `TABLE_INDEXES`, auth login-failure DAO smoke paths, API token store smoke paths, and no local duplicate DDL or index SQL in `app.infra.db.database._create_system_tables()`.
...
- Focused session bootstrap test: run `ensure_session_table()` against a temporary database and assert registry-backed `sessions` creation, registered expiry index, create/get/update/delete/clear/cleanup DAO smoke paths, and no local duplicate session table or index DDL in the DAO source.
- Focused audit-log bootstrap test: run `ensure_audit_table()` against a temporary database and assert registry-backed `audit_logs` creation, registered audit-log indexes, insert/list/stats/cleanup DAO smoke paths, and no local duplicate audit-log table or index DDL in the DAO source.
...
- Focused notification bootstrap test: run selected notification/message bootstrap helpers against a temporary database and assert registry-backed table creation, registered request-admin-message index creation, announcement DAO smoke paths, and no local duplicate registry-owned table or index DDL in the DAO source.
...
- Focused media-request bootstrap test: run `ensure_media_request_schema()` against a temporary database and assert registry-backed `media_requests`, `request_users`, and `media_feedback` creation, registered compatible ALTER application, selected feedback DAO smoke paths, preserved local high-risk media request rebuild migrations, and no local duplicate registry-owned media-request DDL/ALTER in the DAO source.
