...
- Good: `bot_service.py` keeps Emby availability checks, Telegram message editing, notification dispatch, and text rendering in the service while delegating request-admin message sync persistence, notify-rule/mute reads/bootstrap, and media request completion status persistence to DAO boundaries.
