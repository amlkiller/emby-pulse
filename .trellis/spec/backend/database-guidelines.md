...
- Focused database-init compatibility test: run `init_db(skip_migration=True)` against temporary system and playback database paths and assert compatibility `point_logs` / `point_config` plus late message-table creation comes from registry metadata with no local duplicate core point or message DDL in `init_db()`.
